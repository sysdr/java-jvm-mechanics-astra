...
echo -e "${BLUE}======================================================================${NC}"
echo -e "${BLUE}               AstraKV Cleanup (Day 2)                                ${NC}"
echo -e "${BLUE}======================================================================${NC}"
...
# ---------------------------------------------------------------------------
# 1) Stop local AstraKV / collision-test Java processes
# ---------------------------------------------------------------------------
echo -e "\n${CYAN}[1/5] Stopping local AstraKV processes...${NC}"
if pgrep -f 'CollisionTest|AstraKVStore' >/dev/null 2>&1; then
    pkill -f 'CollisionTest|AstraKVStore' 2>/dev/null || true
...
if DOCKER_CMD="$(resolve_docker)"; then
    ...
    ids="$(
        { $DOCKER_CMD ps -aq --filter name=astrakv 2>/dev/null || true
          $DOCKER_CMD ps -aq --filter name=day2 2>/dev/null || true
        } | sort -u | tr '\n' ' '
    )"
    ids="$(echo "$ids" | xargs)"
    if [ -n "${ids:-}" ]; then
